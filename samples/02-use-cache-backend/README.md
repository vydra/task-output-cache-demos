...
* One of the main points of using the same cache backend with builds executed on different hosts is that one build can reuse the results from builds that were executed other machines. For now this will only work if the input file paths are exactly the same on each machine. This practically means that if you ran the build in the `/Users/lptr/Workspace/gradle-task-cache` directory on host A, you'll need to run it from the exact same directory on host B as well for caching to work. This limitation will be lifted once issue #18 is implemented.
